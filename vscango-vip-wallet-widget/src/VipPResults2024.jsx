// VipPResults2024.jsx — COMPLETE COPY/PASTE REPLACEMENT
...
const ENDPOINT = "https://cache.vscango.com/api/2024vipPResults";
...
/**
 * ✅ FIX: Percent handling that respects whether the RAW value already contained a '%'
 * - If raw string contained '%', the numeric is already in percent units (0.56% => 0.56)
...
export default function VipPResults2024() {
  ...
  // ✅ helper to keep focused rows opaque (no see-through under sticky col)
      ...
      >
        ...
        <div style={{ width: "100%", maxWidth: "none", margin: "0 auto" }}>
          <div style={{ marginBottom: 10 }}>
            ...
            >
              <h1 style={{ margin: 0, fontSize: "1.6rem" }}>2024 VIP(P) Results</h1>
            ...
            <div style={{ marginTop: 2, marginBottom: 16 }}>
              <YearSwitcher currentYear={2024} isMobile={isMobile} />
            </div>
          ...
          {!loading && !error && totalRows > 0 && (
            <div style={{ marginBottom: 8, display: "flex", justifyContent: "flex-end" }}>
              <PaginationBar
                ...
                showingText={`Showing ${(pageSafe - 1) * PAGE_SIZE + 1}-${Math.min(pageSafe * PAGE_SIZE, totalRows)} of ${totalRows.toLocaleString()}`}
              />
            </div>
          )}
                          ...
                          <tr key={rowIdx} data-focus-row={isFocusRow ? "1" : undefined} style={{ height: TABLE_BODY_ROW_HEIGHT }}>
                            {columns.map((col, idx) => {
                              ...
                              const baseTd = {
                                ...
                                // ✅ opaque base + optional overlay highlight
                                ...cellBgStyle(baseBg, isFocusRow),

                                ...(isFocusRow ? focusEdgeStyle({ isFirst: idx === 0, isLast: idx === columns.length - 1 }) : {}),
